...
| type | when | target |
|---|---|---|
| `new-hook` | a rule with `status: over_threshold` and `enforcement: prose` (`escalate_to: hook-warn`) | new `.claude/hooks/<rule>-warn.py` + wiring in `.claude/settings.json` |
...
- Import `should_skip` from `_coograph_guard` like every other hook, and emit a `scope`-style signal through `_coograph_signals` (`rule: graph-first`, `detector: graph-first`, `origin: hook`, evidence `{count: 1, first_index: 0, tools: [<tool>], proof: "hook"}`) so the next retro measures it.
