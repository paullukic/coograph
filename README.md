...
| **`no-new-deps-warn.py`** | PreToolUse (Bash, Edit/Write) | **Warns** (non-blocking) the first time a session installs a named package or edits a dependency manifest. Silent for restores (`npm install` bare, `pip install -r`, `pip install -e .`), matching the `new-dependency` detector. Records the warning as a Retro signal. |
| **`defect-warn.py`** | PreToolUse (Bash, Edit/Write, Skill) | **Warns** (non-blocking) once, at `git commit`, when a session edited source and no review or verify skill ran. A proxy for the `defect` rule: a hook cannot see a future fix commit, so it flags the review that was skipped. Markdown and `openspec/` edits do not count. |
